...
def post_finish(upload_id):
  ...
  logger.debug(f'Got trace for upload {upload_id} with trace ID {trace_id} and parent span ID {parent_span_id}')
  
  ps_api_controller.stop_span_for_trace(trace_id, parent_span_id, 'dex-upload')
  logger.debug(f'Stopped child span for parent span {parent_span_id} with stage name of dex-upload')
