...
func OnUploadComplete(flags cliflags.Flags, appConfig appconfig.AppConfig, allUploadConfig metadatav1.AllUploadConfigs, event tusd.HookEvent) error {

	// LOCAL
	if flags.Environment == cliflags.ENV_LOCAL {

		// making only one local copy as concept from tusd raw to a local folder
		lc := localFromTusCopier{
			appConfig:    appConfig,
			uploadConfig: allUploadConfig,
			tusdEvent:    event,
		} // .lc
