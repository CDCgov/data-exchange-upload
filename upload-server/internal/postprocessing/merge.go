...
func RouteAndDeliverHook(publisher event.Publisher[*event.FileReady]) func(handler.HookEvent, hooks.HookResponse) (hooks.HookResponse, error) {
	return func(evt handler.HookEvent, resp hooks.HookResponse) (hooks.HookResponse, error) {
		ctx := context.TODO()
		id := evt.Upload.ID
		meta := evt.Upload.MetaData
		...
		for _, target := range routeGroup.DeliveryTargets {
			...
			e := event.NewFileReadyEvent(id, meta, path, target.Name)
